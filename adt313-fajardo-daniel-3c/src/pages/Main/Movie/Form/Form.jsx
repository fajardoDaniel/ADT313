...
const Form = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.popularity < 0) errors.popularity = 'Popularity cannot be negative';
    if (formData.voteAverage < 0 || formData.voteAverage > 10) {
      errors.voteAverage = 'Vote average must be between 0 and 10';
    }
  ...
  const handleSave = async () => {
    try {
      ...
      console.error('Error saving movie:', error);
      setError(`Failed to ${movieId ? 'update' : 'save'} movie. Please try again.`);
    }
  };
  ...
  const handleUpdate = async () => {
    try {
      if (!validateForm()) {
        setError('Please correct the errors before updating.');
        return;
      }
        ...
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          <div className='field'>
            ...
            {validationErrors.overview && (
              <span className="error-message">{validationErrors.overview}</span>
            )}
          </div>
          <div className='field'>
            Popularity:
            <input
              ...
              onChange={handleInputChange}
            />
            {validationErrors.popularity && (
              <span className="error-message">{validationErrors.popularity}</span>
            )}
          </div>
          <div className='field'>
            Release Date:
          ...
          <div className='field'>
            ...
            <input
              ...
              step="0.1"
            />
            {validationErrors.voteAverage && (
              <span className="error-message">{validationErrors.voteAverage}</span>
            )}
          </div>
          <button 
            type='button' 
